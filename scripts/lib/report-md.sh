...
generate_md_report() {
  ...
  # Portfolio (multi-project) reports render a dedicated roll-up layout.
  if [[ "$(jq -r '.metadata.reportType // "single"' "$report_data_file")" == "portfolio" ]]; then
    generate_md_portfolio_report "$report_data_file" "$output_dir"
    return $?
  fi
...
# ---------------------------------------------------------------------------
# generate_md_portfolio_report <report_data_json> <output_dir>
#   Renders a portfolio roll-up (org totals, gate counts, worst offenders,
#   per-project comparison, and per-project drill-down) as Markdown.
# ---------------------------------------------------------------------------
generate_md_portfolio_report() {
  local report_data_file="$1"
  local output_dir="$2"

  local timestamp
  timestamp=$(date '+%Y%m%d_%H%M%S')
  local filepath="${output_dir}/portfolio_${timestamp}.md"
  mkdir -p "$output_dir"

  jq -r -f "${_REPORT_MD_SCRIPT_DIR}/jq/md-portfolio.jq" "$report_data_file" > "$filepath" || {
    log_error "Failed to generate portfolio Markdown report"
    return 1
  }
