...
main() {
  ...
  for fmt in "${formats[@]}"; do
    ...
    case "$fmt" in
      json)
        local f
        f=$(generate_json_report "$report_data" "$REPORT_OUTPUT_DIR")
        generated_files+=("$f")
        ;;
      md|markdown)
        local f
        f=$(generate_md_report "$report_data" "$REPORT_OUTPUT_DIR")
        generated_files+=("$f")
        ;;
      html)
        html_file=$(generate_html_report "$report_data" "$REPORT_OUTPUT_DIR")
